...
this.setBorder();tinymce.each(['margin','padding','text_padding','text_margin'],function(k){self.setSpacing(k,true);});this.updateText();this.updateCaption();},insert:function(){tinyMCEPopup.restoreSelection();var ed=tinyMCEPopup.editor,el,s=ed.selection,n=s.getNode(),c,w,txt,v,p;var ce={style:ed.dom.serializeStyle(ed.dom.parseStyle($('#caption').get(0).style.cssText)),'class':$('#classes').val()};if(n.nodeName=='IMG'){w=n.width;var styles=ed.dom.parseStyle(n.style.cssText);tinymce.each(['margin','float','padding'],function(k){if(styles[k])
...
var w=ed.dom.getAttrib(n,'width')||n.width;if(w){ed.dom.setStyle(c,'max-width',w+'px');}}
