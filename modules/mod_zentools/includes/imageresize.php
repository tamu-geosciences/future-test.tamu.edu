...
if (!(class_exists('ZenToolsResizeImageHelper'))) {
	class ZenToolsResizeImageHelper
	{
		public static function getResizedImage($image, $newWidth, $newHeight, $option='crop', $quality='90')
		{
			...
			$cache_dir = $site_dir.'media/mod_zentools/cache/images/';

			if (strtolower(substr(PHP_OS, 0, 3)) === 'win') {//windows
				$site_root = $jpath_win.'\\';
				$cache_root = $site_root.'media\\mod_zentools\\cache\\images\\';
				$imageWindows = str_replace('/', '\\', $image);
			...
			$extension = '.'.strtolower(JFile::getExt($image_file));
			$name = JFile::stripExt($image_file);

			unset($jpath, $full_site_dir, $jpath_win, $replace, $image_file);
			...
			if ($img !== false) {
				...
				$new_image = $name.'-'.md5($image.'-'.$newWidth.'x'.$newHeight.'-'.$option.'-'.$lastmod);
				...
				if (($width<$newWidth)&&($height<$newHeight)){
					...
					$local_image = str_replace(JPATH_ROOT, '', $local_image);
				...
				unset($crop, $cropStartX, $cropStartY, $newWidth, $newHeight, $img, $optimalHeight, $option, $optionArray, $cache_root);
