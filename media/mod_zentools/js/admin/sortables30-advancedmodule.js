...
(function( $ ) {
	...
	$.fn.setAccordion = function() {
		...
		$(text + ',' + image + ',' + more  + ',' + titlePanel).parent().show();
	...
	$.fn.updateLink = function() {
		...
		switch ($(linkselect).text()) {
			case 'None':
				jQuery(altlinks).parent().parent().hide();
				jQuery(altlinkslbl).parent().parent().hide();
				jQuery(linktarget).parent().parent().hide();
				jQuery(linktargetlbl).parent().parent().hide();
			break;
	...
	$.fn.layoutSwitch= function() {
		...
		if((jQuery(k2Source).is(":selected")) && (jQuery("#sortable li#image").length == 1) ) {
			...
			jQuery(k2imagetypelbl + ',' + k2imagetype + ',' + k2imageoptions).parent().parent().show();
		}
		else {
			jQuery(k2imagetypelbl + ',' + k2imagetype + ',' + k2imageoptions).parent().parent().hide();
		}
	...
	$.fn.layoutSwitchColumns = function() {
		...
		{
			jQuery(twitterPanel).parent().show();
		}
	};
})( jQuery );
